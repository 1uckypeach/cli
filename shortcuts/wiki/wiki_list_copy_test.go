...
func TestWikiNodeCopyMountedExplainsResourcePermissionDenied(t *testing.T) {
	...
	if !strings.Contains(p.Hint, "container edit permission") || !strings.Contains(p.Hint, "Do not retry the same request") {
		t.Fatalf("hint = %q, want non-retryable write-container guidance", p.Hint)
	}
	if strings.Contains(p.Hint, "grant read access") {
		t.Fatalf("hint = %q, must not prescribe read access for copy", p.Hint)
	}
}
