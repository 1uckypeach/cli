...
const (
	...
	minutesWordReplaceWordsNotFound    = 40013

	minutesWordReplaceDoNotRetrySucceeded = "Do not reprocess words that already succeeded."
)
...
func minutesWordReplaceAllFailed(minuteToken string, failed []string) error {
	...
	return errs.NewAPIError(errs.SubtypeNotFound, "%s", msg).
		WithCode(minutesWordReplaceWordsNotFound).
