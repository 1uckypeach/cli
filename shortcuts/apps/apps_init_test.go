...
func TestCommandLabel(t *testing.T) {
	cases := []struct {
		...
		{"git", []string{"clone", "--", "https://h/r.git", "d"}, "git clone"},
		{"git", []string{"checkout", "sprint/default"}, "git checkout"},
		{"git", []string{"ls-files"}, "git ls-files"},
		{"git", nil, "git"},
		{"npx", []string{"-y", "--prefer-online", "--registry", "u", "pkg", "app", "init", "--app-id", "x"}, "npx app init"},
		{"npx", []string{"-y", "--prefer-online", "--registry", "u", "pkg", "app", "sync"}, "npx app sync"},
		{"npx", []string{"-y", "pkg", "skills", "sync", "--local"}, "npx skills sync"},
		{"npx", []string{"-y", "pkg"}, "npx"},
		{"/tmp/go-build123/lark-cli", []string{"apps", "+git-credential-init", "--app-id", "x", "--format", "json"}, "subprocess +git-credential-init"},
		{"/usr/local/bin/lark-cli", []string{"apps", "+env-pull", "--app-id", "x"}, "subprocess +env-pull"},
		{"weirdtool", nil, "weirdtool"},
	}
	for _, c := range cases {
		if got := commandLabel(c.name, c.args); got != c.want {
			t.Errorf("commandLabel(%q, %v) = %q, want %q", c.name, c.args, got, c.want)
		}
	}
}

func TestInitTimer_SpansSkipTotalInOrder(t *testing.T) {
	base := time.Unix(1_700_000_000, 0)
	var calls int
	clock := func() time.Time {
		tm := base.Add(time.Duration(calls) * 10 * time.Millisecond)
		calls++
		return tm
	}
	tm := &initTimer{now: clock, start: base}

	tm.span("a")() // 10ms (two clock reads: +0, +10)
	tm.skip("b")   // no clock reads
	tm.span("c")() // 10ms (+20, +30)
	tm.finish()    // total = (+40) - base = 40ms

	spans := tm.payload()
	if len(spans) != 4 {
		t.Fatalf("got %d spans, want 4: %v", len(spans), spans)
	}
	wantStep := []string{"a", "b", "c", "total"}
	for i, w := range wantStep {
		if spans[i]["step"] != w {
			t.Errorf("span[%d].step = %v, want %s", i, spans[i]["step"], w)
		}
	}
	if spans[0]["ms"] != int64(10) || spans[2]["ms"] != int64(10) {
		t.Errorf("a/c ms = %v/%v, want 10/10", spans[0]["ms"], spans[2]["ms"])
	}
	if spans[3]["ms"] != int64(40) {
		t.Errorf("total ms = %v, want 40", spans[3]["ms"])
	}
	if spans[1]["skipped"] != true || spans[1]["ms"] != int64(0) {
		t.Errorf("skip span = %v, want ms=0 skipped=true", spans[1])
	}
	// A non-skipped span must not carry a "skipped" key.
	if _, ok := spans[0]["skipped"]; ok {
		t.Errorf("non-skipped span should omit skipped key: %v", spans[0])
	}
}

func TestInitTimer_NilIsNoop(t *testing.T) {
	var tm *initTimer
	tm.span("x")() // must not panic
	tm.skip("y")
	tm.finish()
	if tm.payload() != nil {
		t.Error("nil timer payload should be nil")
	}
}

// timingSteps extracts the ordered step names and the set of skipped steps from
// a parsed envelope's data.timings array.
func timingSteps(t *testing.T, data map[string]interface{}) (order []string, skipped map[string]bool) {
	t.Helper()
	raw, ok := data["timings"].([]interface{})
	if !ok {
		t.Fatalf("data.timings missing or not an array: %#v", data["timings"])
	}
	skipped = map[string]bool{}
	for _, e := range raw {
		m, ok := e.(map[string]interface{})
		if !ok {
			t.Fatalf("timing entry not an object: %#v", e)
		}
		step, _ := m["step"].(string)
		order = append(order, step)
		if s, _ := m["skipped"].(bool); s {
			skipped[step] = true
		}
	}
...
func TestAppsInit_Timings_FreshClone(t *testing.T) {
	f := &fakeCommandRunner{results: map[string]fakeCallResult{
		"credential-init": credInitOK("https://git.test/app_tm.git"),
		"git clone":       {},
		"git checkout":    {},
		"git ls-files":    {stdout: ""},
		"npx -y":          {},
	...
	reg.Register(&httpmock.Stub{
		Method: "GET",
		URL:    "/open-apis/spark/v1/apps/app_tm",
	...
	dir := relCloneDir(t)
	if err := runAppsShortcut(t, AppsInit, []string{"+init", "--app-id", "app_tm", "--dir", dir, "--as", "user"}, factory, stdout); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	data := parseEnvelopeData(t, stdout)
	order, skipped := timingSteps(t, data)

	idx := map[string]int{}
	for i, s := range order {
		if _, dup := idx[s]; !dup { // record first occurrence
			idx[s] = i
		}
	}
	for _, want := range []string{
		"query_app_type", "subprocess +git-credential-init", "git clone",
		"git checkout", "git ls-files", "npx app init", "git status", "git push", "total",
	} {
		if _, ok := idx[want]; !ok {
			t.Errorf("timings missing step %q; got order %v", want, order)
		}
	}
	// env-pull is policy-skipped for modern_html and must be recorded as skipped.
	if !skipped["env_pull"] {
		t.Errorf("env_pull should be recorded skipped; skipped=%v order=%v", skipped, order)
	}
	// Ordering sanity: query precedes clone; total is last.
	if idx["query_app_type"] >= idx["git clone"] {
		t.Errorf("query_app_type must precede git clone; order=%v", order)
	}
	if order[len(order)-1] != "total" {
		t.Errorf("total must be the last span; order=%v", order)
	}
}
