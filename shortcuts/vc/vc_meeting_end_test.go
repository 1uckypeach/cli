...
func TestMeetingEndContract(t *testing.T) {
	...
	if len(VCMeetingEnd.AuthTypes) != 1 || VCMeetingEnd.AuthTypes[0] != "user" {
		t.Fatalf("AuthTypes = %v, want [user]", VCMeetingEnd.AuthTypes)
	}
}
...
func TestMeetingEndDryRunUsesEscapedPathWithoutAPICall(t *testing.T) {
	...
	if call.Method != "PATCH" || call.URL != "/open-apis/vc/v1/meetings/7651377260537433044/end" {
		t.Fatalf("dry-run call = %#v", call)
	}
	if call.Body != nil {
		t.Fatalf("dry-run body = %#v, want nil/omitted", call.Body)
	}
}
