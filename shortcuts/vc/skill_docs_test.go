...
func TestVCMeetingManagementDocsMatchAuthTypesAndContracts(t *testing.T) {
	...
	for _, cmd := range []struct {
		...
		t.Run(cmd.name, func(t *testing.T) {
			if hasAuthType(cmd.authTypes, "bot") || !hasAuthType(cmd.authTypes, "user") {
				t.Fatalf("%s AuthTypes = %v, want user-only", cmd.name, cmd.authTypes)
			}
			if !strings.Contains(skill, "references/"+cmd.reference) {
			...
			reference := readSkillDoc(t, "skills/lark-meeting/references/"+cmd.reference)
			if !strings.Contains(reference, "仅支持 `user` 身份") && !strings.Contains(reference, "必须显式使用 `--as user`") {
