...
func assertSlidesMissingScopeTerminal(t *testing.T, err error, scope string) {
	...
	if p.Retryable {
		t.Fatalf("missing_scope must not be retryable: %#v", p)
	}
...
func TestAnnotateSlidesMissingScope(t *testing.T) {
	...
	again := annotateSlidesMissingScope(got)
	if p2, _ := errs.ProblemOf(again); strings.Count(p2.Hint, slidesMissingScopeTerminalHint) != 1 {
