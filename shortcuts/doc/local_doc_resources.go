...
func validateLocalDocResourceUpdateCommand(command string, resources []localDocResource) error {
	if len(resources) == 0 || command == "append" || command == "block_insert_after" {
		return nil
	}
	return errs.NewValidationError(
		errs.SubtypeInvalidArgument,
		"local images and files are only supported with --command append or block_insert_after so a bind failure cannot destroy existing content",
	).WithParams(
		errs.InvalidParam{Name: "--command", Reason: "use append or block_insert_after for local resources"},
...
func finalizeLocalDocResources(runtime *common.RuntimeContext, documentKey string, data map[string]interface{}, resources []localDocResource) error {
	...
	if strings.TrimSpace(documentKey) == "" {
		...
		return emitLocalDocResourcePartialFailure(runtime, data, outcomes)
	}
...
func uploadLocalDocResources(runtime *common.RuntimeContext, documentKey string, outcomes []*localDocResourceOutcome) {
	...
	for _, outcome := range outcomes {
		...
		for attempt := 0; attempt < localDocResourceUploadAttempts; attempt++ {
			token, err := uploadDocMediaFile(runtime, UploadDocMediaFileConfig{
				...
				DocID:              documentKey,
				MinRequestInterval: localDocResourceUploadInterval,
			})
			if err == nil {
			...
			uploadErr = err
			if !isRetryableLocalDocResourceUploadConflict(err) || attempt+1 >= localDocResourceUploadAttempts {
				break
			}
			delay := localDocResourceUploadInterval * time.Duration(1<<attempt)
...
func isRetryableLocalDocResourceUploadConflict(err error) bool {
...
func emitLocalDocResourcePartialFailure(runtime *common.RuntimeContext, data map[string]interface{}, outcomes []*localDocResourceOutcome) error {
	if data == nil {
		data = map[string]interface{}{}
	}
	...
	for _, outcome := range outcomes {
		item := map[string]interface{}{
			...
			"block_type":     string(outcome.Resource.Kind),
			"status":         outcome.Status,
			"cleanup_status": outcome.CleanupStatus,
		}
		if outcome.BlockID != "" {
			item["block_id"] = outcome.BlockID
		}
		if outcome.FileToken != "" {
		...
		if outcome.Status == "bound" {
			summary["succeeded"] = summary["succeeded"].(int) + 1
		} else {
			summary["failed"] = summary["failed"].(int) + 1
		}
		if outcome.CleanupStatus == "succeeded" {
			summary["cleaned"] = summary["cleaned"].(int) + 1
		}
		if outcome.CleanupStatus == "failed" {
			summary["cleanup_failed"] = summary["cleanup_failed"].(int) + 1
		}
		if outcome.Status == "bind_ambiguous" {
			summary["ambiguous"] = summary["ambiguous"].(int) + 1
		}
	}
...
func sanitizedLocalDocResourceError(runtime *common.RuntimeContext, err error, resource localDocResource) map[string]interface{} {
	...
	if problem, ok := errs.ProblemOf(err); ok {
		result["type"] = problem.Category
		result["subtype"] = problem.Subtype
		if problem.Code != 0 {
			result["code"] = problem.Code
		}
		if problem.LogID != "" {
			result["log_id"] = problem.LogID
		}
