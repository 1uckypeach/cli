...
func TestDocMediaParentType(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "native docx block keeps requested parent type",
			parentType: "docx_image",
			parentNode: "blkcnNative123",
			want:       "docx_image",
		},
		{
			name:       "local office prefix alone is not a local token",
			parentType: "docx_file",
			parentNode: "local_office_abc123",
			want:       "docx_file",
		},
		{
			name:       "obsolete fake office prefix is not a local token",
			parentType: "docx_image",
			parentNode: "fake_office_abc123",
			want:       "docx_image",
		},
		{
			name:       "interleaved OFL0X local token",
			parentType: "docx_image",
			parentNode: "KvLqOjiJMFwICuLfVeK0z3LTXNf3",
			want:       officeDocxFileParentType,
		},
		{
			name:       "interleaved OFL0X excel token is not a doc token",
			parentType: "docx_image",
			parentNode: "KvLqOjiJMFwICuLfVeK0z3LTXNf1",
			want:       "docx_image",
		},
		{
			name:       "interleaved OFL0X ppt token is not a doc token",
			parentType: "docx_file",
			parentNode: "KvLqOjiJMFwICuLfVeK0z3LTXNf2",
			want:       "docx_file",
		},
		{
			name:       "interleaved OFL0X token with unknown type is not a doc token",
			parentType: "docx_image",
			parentNode: "aaaaOaaaaFaaaaLaaaa0aaaaXaaa",
			want:       "docx_image",
		},
		{
			name:       "OFL0X marker with short token is not local",
			parentType: "docx_file",
			parentNode: "aaaaOaaaaFaaaaLaaaa0aaaaXaa",
			want:       "docx_file",
		},
	}
...
func TestDocMediaUploadDryRunUsesOfficeParentTypeForLocalToken(t *testing.T) {
	cmd := &cobra.Command{Use: "docs +media-upload"}
	cmd.Flags().String("file", "", "")
	cmd.Flags().String("parent-type", "", "")
	cmd.Flags().String("parent-node", "", "")
...
func TestDocMediaUploadExecuteUsesOfficeParentTypeForLocalToken(t *testing.T) {
