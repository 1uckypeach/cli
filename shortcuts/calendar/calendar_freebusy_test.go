...
func TestFreebusy_CommonFreeView(t *testing.T) {
	...
	// Expect free slots: 10-11 (1h), 12-18 (6h)
	...
	if payload.Data.CommonFree[0].StartTime[:19] != "2026-03-21T10:00:00" ||
		payload.Data.CommonFree[0].EndTime[:19] != "2026-03-21T11:00:00" {
		t.Errorf("first common-free slot = %+v", payload.Data.CommonFree[0])
	}
	if payload.Data.CommonFree[1].StartTime[:19] != "2026-03-21T12:00:00" ||
		payload.Data.CommonFree[1].EndTime[:19] != "2026-03-21T18:00:00" {
		t.Errorf("second common-free slot = %+v", payload.Data.CommonFree[1])
	}
}
...
func TestFreebusy_FreeView_OffHoursHint(t *testing.T) {
	...
	// Window covers 06:00-09:00; free slot will include 06:00-09:00, hitting
	// the early-morning off-hours band.
	err := mountAndRun(t, CalendarFreebusy, []string{
		"+freebusy",
	...
	if !strings.Contains(stderr.String(), "+suggestion") {
		t.Errorf("expected hint to route to +suggestion, got: %q", stderr.String())
	}
}
...
func TestFreebusy_CommonFree_OffHoursHint(t *testing.T) {
	...
	err := mountAndRun(t, CalendarFreebusy, []string{
		"+freebusy",
		"--start", "2026-03-21T21:00:00+08:00",
		"--end", "2026-03-21T23:30:00+08:00",
		"--user-id", "ou_a,ou_b",
		"--type", "common_free",
		"--as", "bot",
	}, f, stdout)
	...
	if !strings.Contains(stderr.String(), "+suggestion") {
		t.Errorf("expected +suggestion routing hint on stderr, got: %q", stderr.String())
	}
}
