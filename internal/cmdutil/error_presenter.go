...
func (f *Factory) PresentError(err error, options ErrorPresentationOptions) error {
	...
	if typed, ok := errs.UnwrapTypedError(rendered); ok {
		if paginationErr, ok := typed.(*errs.PaginationError); ok { //nolint:errorlint
			completePermissionRecovery(f, paginationErr.Cause, projector, options.Identity, options.DeclaredScopes)
			applyNeedAuthorizationHint(paginationErr.Cause, projector, options.DeclaredScopes)
