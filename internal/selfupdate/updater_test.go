	...
	tests := []struct {
	...
	}{
		{
			...
			run: func(u *Updater) *NpmResult {
				return u.runSkillsListOfficial("https://open.feishu-boe.cn")
			},
			want: "-y skills add https://open.feishu-boe.cn --list",
		},
		{
			name: "list global",
			run: func(u *Updater) *NpmResult {
		...
		},
		{
			...
			run: func(u *Updater) *NpmResult {
				return u.runSkillsInstall("https://open.feishu-boe.cn", []string{"lark-mail"})
			},
			want: "-y skills add https://open.feishu-boe.cn -s lark-mail -g -y",
		},
	}
...
func TestListOfficialSkillsFallsBack(t *testing.T) {
	...
	updater := &Updater{
		SkillsCommandOverride: func(args ...string) *NpmResult {
			...
			r := &NpmResult{}
			if strings.Contains(strings.Join(args, " "), "https://open.feishu-boe.cn") {
...
func TestSkillsInvocation(t *testing.T) {
	addArgs := []string{"-y", "skills", "add", "https://open.feishu-boe.cn", "-g", "-y"}
	cases := []struct {
		name          string
		...
		{"pnpm install + pnpm available → pnpm dlx, drop leading -y", InstallPnpm, true, addArgs,
			"pnpm", []string{"dlx", "skills", "add", "https://open.feishu-boe.cn", "-g", "-y"}},
...
func TestSkillsBrandHosts(t *testing.T) {
	cases := []struct {
		...
		{core.BrandFeishu, "https://open.feishu-boe.cn/.well-known/skills/index.json", "https://open.feishu-boe.cn"},
