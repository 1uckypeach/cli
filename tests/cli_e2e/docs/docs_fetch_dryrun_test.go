...
func TestDocsFetchCommentsRejectsNonXMLBeforeDryRun(t *testing.T) {
	setDocsDryRunEnv(t)
	...
	for _, docFormat := range []string{"markdown", "im-markdown"} {
		t.Run(docFormat, func(t *testing.T) {
			...
			require.Contains(t, gjson.Get(result.Stderr, "error.message").String(), "only XML", result.Stderr)
			require.Contains(t, gjson.Get(result.Stderr, "error.message").String(), "Markdown and IM Markdown remain unchanged", result.Stderr)
		})
	}
}
