...
| `--answer` 缺 `--context-id/--task-id` | invalid_argument | 2 | `回答问题组需同时提供 --context-id 与 --task-id`；hint 指向照抄 meta.next 模板 |
| `--answer` 键/值违规 | invalid_argument | 2 | `非法的 --answer: ...` 一次报全：非 `key=value` 形、key 非法（仅 `<qid>` 与 `<qid>.text` 两种形态，`.txt`/`.TEXT`/`-` 打头都不行）、空值（`空答案无意义`——不想答的题不要带 key）、同题 `.text` 重复 |
