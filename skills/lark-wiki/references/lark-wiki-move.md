...
- **`131006` 是资源权限失败，不是 scope 缺失**：copy/move 写操作需要相关父节点的容器编辑权限；移动到知识空间根目录时，还可能需要成为该空间的成员或管理员。不要用同一请求重试、重新授权或换身份试错。解析源节点 / 目标父节点时的 `get_node` `131006` 仍按只读失败处理：缺的是节点查看权限，不是容器编辑权限。
