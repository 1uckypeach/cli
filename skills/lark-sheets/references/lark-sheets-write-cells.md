...
> **数字还是文本，按"是否参与计算"二选一**：要求和 / 排序 / 比较大小 / 按数值格式显示的真实数值，必须以**数字类型**写入、且**不要**把该列设成 `@` 文本格式，否则会被当文本、丧失计算能力（`+table-put` 用 `dtypes` 声明 `int64` / `float64`，别在本地拼成带 `$` / `%` 的字符串走 `+csv-put`）。反过来，编号 `001`、规格 `3-1`、身份证 / 电话 / 单据号等**要原样保留、不想被飞书自动解释**的内容（否则 `001`→`1`、`3-1`→日期、长号→科学计数），必须以**字符串类型**写入（`dtypes` 设 `object`）并把 `number_format` 设为 `"@"`（文本格式），才能字面保真。
