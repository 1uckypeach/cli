...
lark-cli agent card <provider>:<agent_id> --operation send

# 一次拿全所有动词的参数契约（要调 2+ 个动词时省往返）
lark-cli agent card <provider>:<agent_id> --operation all
```
