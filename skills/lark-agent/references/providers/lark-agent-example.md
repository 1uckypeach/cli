...
- **参数声明演示（`example:reporter` 的 send）**：声明了两个可选参数作 copy-start 模板——`report_format`（enum: `csv|xlsx`，默认 `csv`）、`quarters`（integer，范围 1..12，默认 `4`）。`agent card example:reporter --operation send` 可看到完整契约；不带参数时行为与历史一致（默认值回填），`--param report_format=xlsx` 会改变回复文案；`--param report_format=pdf` 触发 enum 教学错误（离线、exit 2、列出合法值全集）。另声明 `render` **object 参数**（fields: `theme` enum light|dark 默认 light、`watermark` boolean 默认 false）演示点路径传参——`--param render.theme=dark --param render.watermark=true` 会让回复带"dark 主题，含水印"；JSON 整值 `--param render='{"theme":"dark","watermark":true}'` 等价。
