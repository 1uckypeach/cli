...
> **默认路由：** 用户需要记录结果、统计、聚合、分析、归因、趋势或报告时，统一调用 `base:assistant`，不要执行本文 CLI 查询。本文只用于用户明确指定 Base CLI、排障，或为目标明确的 CLI 写入执行内部读取；不得把内部读取结果直接当作普通问数答案。路由规则见 [`../SKILL.md`](../SKILL.md)。
