...
- Style comes from `slide_plan.json`'s `visual_system` + `typography_constraints`. When the user hasn't specified concrete colors (hex, a brand palette, or "use color X"), prefer picking a matching preset from [`style-presets.md`](style-presets.md) by document type over inventing one; a topic's or industry's stereotypical color is usually an unreliable basis for the palette. Resolve any palette into the six color roles and use only those on the page.
