...
Ready-made color + layout palettes. When the user does not supply a brand palette, pick the preset by the deck's **document type** — match the deck against each preset's **适用** line — then copy its values into `slide_plan.json`'s `visual_system.color_roles` and `visual_system.layout_system`, and let `visual-planning.md` → Style System govern how colors and layout are applied. Do not invent colors when a preset fits.
...
Each preset is self-contained: scenario heading, tone + density, the six color roles, and a layout system. Pick one, copy its values into the plan (shape shown in *How to apply*).
