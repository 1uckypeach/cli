...
Each preset already fills all six color roles. Where the source palette had no saturated emphasis color, an `accent` has been **derived** (marked ⟲) per the accent-gap rule, so every preset is ready to use as-is.
...
- The ⟲ accents are derived; if the user later supplies a real emphasis color, replace it.
