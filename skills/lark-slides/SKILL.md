...
| 获取幻灯片页面截图 | 指定页用 `--slide-number` / `--slide-id`，一次最多 10 页；整份用显式 `--all`，由 CLI 自动分批，最多 100 页 | `slides +screenshot`、`lark-slides-screenshot.md` |
...
| [`+screenshot`](references/lark-slides-screenshot.md) | 保存页面截图：指定页用 `--slide-number` / `--slide-id`（一次最多 10 页），整份用 `--all`（CLI 自动分批，最多 100 页）；单张用 `--output`，多张用 `--output-dir`，后续读取实际返回路径 |
