...
| 绘制流程图、时序图、架构图、类图、ER 图、散点图等拓扑或自定义图形 | 用 `<whiteboard>` 内嵌 SVG 或 Mermaid，`<chart>`/`<shape>` 覆盖不到的场景才用 | `lark-slides-whiteboard.md` |
...
- 画板：[`lark-slides-whiteboard.md`](references/lark-slides-whiteboard.md)
