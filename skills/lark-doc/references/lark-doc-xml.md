...
- `<whiteboard>` — 按 [`lark-doc-whiteboard.md`](lark-doc-whiteboard.md) 先判断图形语义与复杂度：结构语义强且 Mermaid 适配时可直接写 `<whiteboard type="mermaid">...</whiteboard>`；简单且自包含的 SVG 可直接写 `<whiteboard type="svg">完整自包含 SVG</whiteboard>`；复杂图或需要多轮编辑时先创建 `<whiteboard type="blank"></whiteboard>`，再按该指南切到 `lark-whiteboard` 写入；
