...
4. **不能做会后复盘**，**不能替代参会人快照查询**。如果会议已结束：
   - 先用 `lark-cli vc +detail --as <沿用上游身份> --meeting-ids <meeting.id>` 获取会议产物信息；身份与上一步保持一致，应用机器人入会拿到的 `meeting_id` 用 `--as bot`，用户身份发现的用 `--as user`。
...
#    仍沿用入会身份：meeting.get 只返回当前身份参与过的会议
lark-cli vc +detail --as "$AS" --meeting-ids "$MID"
```
