...
func TestBrandFilter_AppsExcludedOnLark(t *testing.T) {
	feishuDomains := allKnownDomains(authTestCatalog(t), core.BrandFeishu)
	if !feishuDomains["apps"] {
		t.Errorf("expected apps domain to be known on Feishu brand")
	}

	larkDomains := allKnownDomains(authTestCatalog(t), core.BrandLark)
	if larkDomains["apps"] {
		t.Errorf("expected apps domain to be EXCLUDED on Lark brand")
	}

	feishuScopes := collectScopesForDomains(authTestCatalog(t), []string{"apps"}, "user", core.BrandFeishu)
	if len(feishuScopes) == 0 {
		t.Errorf("expected non-empty scopes for apps on Feishu brand, got %d", len(feishuScopes))
	}

	larkScopes := collectScopesForDomains(authTestCatalog(t), []string{"apps"}, "user", core.BrandLark)
	if len(larkScopes) != 0 {
