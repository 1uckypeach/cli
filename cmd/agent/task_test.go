...
// taskCmdCtx builds a `lark-cli agent task <leaf>` command whose CommandPath()
...
func TestTaskGetSingle(t *testing.T) {
	...
	setScripted(t, scriptedHooks{getTask: func(taskID string) (*iagent.AgentTask, error) {
		return &iagent.AgentTask{TaskID: taskID, ContextID: "sess_1", State: iagent.StateCompleted, IsTerminal: true}, nil
	}})
	...
	if data["state"] != string(iagent.StateCompleted) {
...
func TestTaskGetError(t *testing.T) {
	opts, _ := taskTestOpts(t, "get")
	setScripted(t, scriptedHooks{getTask: func(string) (*iagent.AgentTask, error) {
...
func TestTaskGetWatchTerminalFailureExit(t *testing.T) {
	...
	opts.Watch = true
	setScripted(t, scriptedHooks{getTask: func(taskID string) (*iagent.AgentTask, error) {
		return &iagent.AgentTask{TaskID: taskID, State: iagent.StateFailed, IsTerminal: true}, nil
	}})
...
func TestTaskGetWatchTerminalSuccessExit(t *testing.T) {
	...
	opts.Watch = true
	setScripted(t, scriptedHooks{getTask: func(taskID string) (*iagent.AgentTask, error) {
		return &iagent.AgentTask{TaskID: taskID, State: iagent.StateCompleted, IsTerminal: true}, nil
	}})
	out := opts.Factory.IOStreams.Out.(interface{ Bytes() []byte })
...
func TestTaskGetWatchBoundedTimeout(t *testing.T) {
	...
	setScripted(t, scriptedHooks{getTask: func(taskID string) (*iagent.AgentTask, error) {
		return &iagent.AgentTask{TaskID: taskID, State: iagent.StateWorking}, nil
	}})
	out := opts.Factory.IOStreams.Out.(interface{ Bytes() []byte })
...
func TestTaskListEmitsCount(t *testing.T) {
	...
	setScripted(t, scriptedHooks{listTasks: func(contextID string) ([]iagent.TaskSummary, error) {
		return []iagent.TaskSummary{
			{TaskID: "chat_1", State: iagent.StateCompleted, IsTerminal: true},
			{TaskID: "chat_2", State: iagent.StateWorking},
		}, nil
	}})
	out := opts.Factory.IOStreams.Out.(interface{ Bytes() []byte })
...
func TestTaskGetArtifactURLDownload(t *testing.T) {
	...
	setScripted(t, scriptedHooks{downloadArtifact: func(taskID, artifactID string) (*iagent.ArtifactData, error) {
		return &iagent.ArtifactData{Name: "../escape.txt", URL: srv.URL}, nil
	}})
...
func TestTaskGetArtifactInlineWrite(t *testing.T) {
	...
	restoreP := swapResolveDownload(func(*taskOptions) (*iagent.ArtifactData, error) {
		return &iagent.ArtifactData{Mime: "application/octet-stream", Bytes: []byte("INLINE")}, nil
	})
	defer restoreP()
...
func TestTaskGetPrettyFormat(t *testing.T) {
	...
	opts.Format = "pretty"
	setScripted(t, scriptedHooks{getTask: func(taskID string) (*iagent.AgentTask, error) {
		return &iagent.AgentTask{
			TaskID: taskID, ContextID: "sess_1", State: iagent.StateCompleted, IsTerminal: true,
			Messages: []iagent.Message{{Role: "agent", Parts: []iagent.Part{
				{Type: "text", Text: "\x1b[31m分析完成\x1b[0m"},
			}}},
			Artifacts: []iagent.Artifact{{ID: "art_1", Kind: "file"}},
		}, nil
	}})
	out := opts.Factory.IOStreams.Out.(interface{ Bytes() []byte })
...
func swapResolveDownload(fn func(*taskOptions) (*iagent.ArtifactData, error)) func() {
...
func TestDownloadArtifact_SafeOutputPathError(t *testing.T) {
	fetched := false
	restore := swapResolveDownload(func(*taskOptions) (*iagent.ArtifactData, error) {
		fetched = true
		return &iagent.ArtifactData{Bytes: []byte("x")}, nil
	})
	defer restore()
...
func TestDownloadArtifact_URLFetchError(t *testing.T) {
	restore := swapResolveDownload(func(*taskOptions) (*iagent.ArtifactData, error) {
		return &iagent.ArtifactData{Mime: "text/plain", URL: publicArtifactURL}, nil
	})
	defer restore()
...
func TestDownloadArtifact_RefusesOverwriteWithoutForce(t *testing.T) {
	...
	restore := swapResolveDownload(func(*taskOptions) (*iagent.ArtifactData, error) {
		fetched = true
		return &iagent.ArtifactData{Bytes: []byte("NEW")}, nil
	})
	defer restore()
